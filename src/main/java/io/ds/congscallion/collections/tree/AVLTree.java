...
/**
 *
 * AVL树是自平衡二叉搜索树。在传统的排序二进制搜索树（BST）中，搜索时间与链表（log n）的搜索时间相同。
 * 但是，由于AVL树通过在树变得不平衡时进行旋转来平衡自身，因此保证了O（log n）搜索时间，从而使AVL树在性能方面非常一致。
 *
 * 如果应用程序涉及许多频繁的插入和删除，那么应该首选红黑树。如果插入和删除频率较低并且搜索是更频繁的操作，则AVL树应优先于红黑树。
 *
 *
 * 与Hash表相比：
 * 1、 树可以通过中序遍历(Inorder Traversal)按排列顺序获取所有数据。 Hash表不具备排序的能力，需要提供额外的操作才可以。
 * 2、 树可以优做范围查询，比如统计，查询 x < data < y 某段范围内的数据。  Hash表不具备这样的能力，需要提供额外的操作才可以。
 * 3、 树更易于实现，能够自由地自己实现特定的树逻辑。 但Hash表需要依赖编程语言提供的函数库来计算值。
 * 4、 avl树所有操作都可以保证在 O(log n)时间内完成。 Hash平均时间保证在(O(1))，但某些特定操作可能成本很高，尤其是在表调整大小的时。
 *
 *
 * 树中，节点高度与深度:
 *  高度： 当前节点到子树
 ...
 *
 * AVL树高度相关计算规则：
 *  判断树是否平衡： 左子树和右子树高度差不大于1, 则树是平衡的。
 *  计算节点高度公司如下： height = max (leftSubTreeHeight, rightSubTreeHeight) + 1
 *  判断树是否平衡： | leftSubTreeHeight - rightSubTreeHeight | <= 1
 *
 *　AVL树平衡方法：
 *  　左旋
 *  　右旋
 ...
 *　AVL树插入操作逻辑：　将数据K插入树
 *  　１、　使用二叉搜索树插入方法插入值K。
 *  　２、　从节点K向上查找第一个不平衡节点Z, Y是Z子节点, X是Y的子节点。
 *  　３、　通过在以Z为根的子树上执行适当的旋转来重新平衡树。　一共有如下四种情况:
 *      3.1、　Y是Z的左节点且X是Y的左节点。　(left left case)
 *
