...
/**
 * A binary search tree class with insertion, removal and lookup. A comparator is used to order the
 * items in the tree.  All tree items must be distinct according to the comparator.  If no
 * comparator is supplied the natural order of tree elements is used.
 */
public class BinarySearchTree<E> {
  ...
  /**
   * Comparator used to order the items in the tree.  If null, the natural order of the items will
   * be used.
   */
  ...
  /**
   * Adds a single data item to the tree.  If there is already an item in the tree that compares
   * equal to the item being inserted, it is "overwritten" by the new item.
   */
  ...
  /**
   * Removes the node containing the given value.  Does nothing if there is no such node.
   */
  ...
  /**
   * A special helper method that returns the node containing an object that compares equal to the
   * given object.  This is used in both contains and remove.
   */
