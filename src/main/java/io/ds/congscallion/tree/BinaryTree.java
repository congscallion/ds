...
public class BinaryTree<E extends Comparable> {
   ...
   *
   * 1、如果新节点的值小于当前节点的值，进入该节点的左子树 2、如果新节点的值大小或者等于当前节点的值，进入该节点的右子树 3、当前节点为空时，可以在该位置插入新节点
   *
   ...
   * 删除节点， 删除逻辑如下：
   *
   * 1、被删除节点无子节点时，修改当前节点在其父节点的引用为null即可。 2、被删除节点只有一个子节点时，使用子节点替换当前节点在父节点的引用即可。
   ...
   * 1、先序遍历(pre-order):  对任意节点， 先访问根，其次遍历左子树，最后遍历右子树。 2、中序遍历(in-order): 对任意节点， 先遍历左子树，其次访问根，最后遍历右子树。
