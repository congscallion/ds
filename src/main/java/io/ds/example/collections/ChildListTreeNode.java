...
/**
 * A simple class for tree nodes, implemented with direct links to its parent and its list of
 * children.
 */
public class ChildListTreeNode<E> implements SimpleTreeNode<E> {
  ...
  /**
   * Constructs a node as the root of its own one-element tree. This is the only public constructor.
   * The only trees that clients can make directly are simple one-element trees.
   */
  ...
  /**
   * Returns the children of this node as a list of nodes.  The returned list is read-only to
   * prevent callers from inserting garbage into the actual list of children in the tree!
   */
  ...
  /**
   * Removes child from its current parent and inserts it at the given index of this node.  Indices
   * start at 0.
   *
   * @throws IndexOutOfBoundsException if the index is out of bounds.
   * @throws IllegalArgumentException if the child is an ancestor of this node, since that would
   * make a cycle in the tree.
   * @throws ClassCastException if child is not a ChildListTreeNode.
   */
