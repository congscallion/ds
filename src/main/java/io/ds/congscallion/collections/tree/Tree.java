 ...
 *          2.1.1、完全二叉树：对于一颗二叉树，假设其深度为d（d>1）。
 *                 除了第d层外，其它各层的节点数目均已达最大值，且第d层所有节点从左向右连续地紧密排列，这样的二叉树被称为完全二叉树；
 *          2.1.2、满二叉树：所有叶节点都在最底层的完全二叉树；
 *          2.1.3、平衡二叉树（AVL树）：当且仅当任何节点的两棵子树的高度差不大于1的二叉树；
 *          2.1.4、排序二叉树(二叉查找树(Binary Search Tree))：也称二叉搜索树、有序二叉树；
 *      2.2、自平衡的二叉搜索树
 *          2.2.1、 红黑树
 *          2.2.2、 伸展树（Splay tree）
 *          2.2.3、 树堆（Treap）
 *          2.3.4、 AA树
 *          2.3.5、 2-3树
 *          2.3.6、 2-3-4树
 *          2.3.7、 替罪羊树（Scapegoat tree）
 *
 *  3、霍夫曼树：带权路径最短的二叉树称为哈夫曼树或最优二叉树；
 *  4、B树：一种对读写操作进行优化的自平衡的二叉查找树，能够保持数据有序，拥有多于两个子树。
 *      4.1 B树
 *      4.2 B+树
 *      4.3 B*树
 *      4.4 2-3树
