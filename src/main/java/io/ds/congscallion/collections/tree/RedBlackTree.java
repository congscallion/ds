 ...
 */
public class RedBlackTree<E extends Comparable> extends AbstractBinarySearchTree<E> implements
    Tree {
...
//        remove(a);
   ...
   * <p>
   * 1、规定插入节点为K，父节点为P, 父节点的父节点为G, 父节点的兄弟节点为U, T代表整颗树。 2、标记K为红色。 3、使用普通的二叉搜索树插入方法插入K到正确位置。
   * 4、检查插入K之后，树是否满足红黑树牲。 检查逻辑如下：
   *
   * 4.1.  T是空树，则将K标记为树根，并将其着色为黑色 4.2.  P是黑色，不做任何处理 4.3.  P是红色，根据U的颜色处理
   *
   * 4.3.1. U是红色， 重新着色P，G，U.  即： P，U着黑色，G着红色
   *
   * 4.3.2. U是黑色或NULL， P是G的右子节点，K是P的右子节点 则以G为根左旋子树，使G成为K的兄弟节点S。然后S着红色，P着黑色
   *
   * 4.3.3. U是黑色或NULL， P是G的右子节点，K是P的左子节点 则先以P为根右旋，之后，使用4.3.2描述的规则处理
   *
   * 4.3.4. U是黑色或NULL， P是G的左子节点，K是P的左子节点 则以G为根右旋子树，使G成为K的兄弟节点S。然后S着红色，P着黑色
   *
   * 4.3.5. U是黑色或NULL， P是G的左子节点，K是P的右子节点 则先以P为根左旋，之后，使用4.3.4描述的规则处理
   */
   ...
   *
   * 1. 规定当前检查的节点是K，父节点为P, S代表K的兄弟节点, T代表整颗树。 2. 如果K为红色，无须平衡。 3. 如果K为黑色，且K不等于root时，存在如下８种情况 ４.
   * 如果K为黑色，且在P的左节点。 ４.1 S为红色。　重新着色：　S着黑色，　P着红色，　并以P为根左旋。 左旋完成之后树将进入4.2或４.3或4.4。 ４.2
   * S为黑色，且S左右节点都是黑色。　重新着色：　S着红色。并将K设置为P（k=p）重新检查平衡条件。 ４.3 S为黑色，且S右节点是黑色，S左节点是红色。　重新着色：　S左节点着黑色，S着红色，且以S为根右旋。
   * 右旋完成，树进入4.4。 ４.4 S为黑色，且S右节点是红色。　重新着色：　S着P的颜色，P着黑色，S右节点着黑色，且以P为根左旋。
   * 并将k设置为root（k==root）重新检查平衡条件。
   *
   * ５. 如果K为黑色，且在P的右节点。 平衡操作与４相反。
   *
