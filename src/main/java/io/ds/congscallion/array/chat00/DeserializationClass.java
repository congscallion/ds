...
/**
 *
 * 1. java序列化对象必须实现java.io.Serializable接口
 * 2. java.io.Serializable接口是标记接口，没有什么方法
 * 3. transient修饰的实例属性不会被序列化
 * 4.　父类实现了java.io.Serializable接口，子类也可以序列化
 * 5. 子类继承未实现java.io.Serializable接口的父类，父类所有变量均是零值
 * 6. serialVersionUID字段不是必须的。该字段是每个可序列化类序列化时的版本号，在反序列化时，可用于校验。比如是否兼容等。
 * 7. 如果声明serialVersionUID，类型必须是long且是final.
 * 8. 未显示定义serialVersionUID字段，序列化运行时将计算该类的默认值。　这可能因编译器而异。因此建议定义serialVersionUID.
 * 9. 建议serialVersionUID使用私有访问符
 * 10. 数组类不能声明serialVersionUID，因为它们始终具有默认的计算值，但是对数组类，不需要匹配serialVersionUID值.
